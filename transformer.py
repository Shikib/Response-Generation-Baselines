...
class Transformer(nn.Module):
  ...
  def enc(self, inputs):
    if self.has_inputs:
      i_mask = (inputs == self.src_pad_idx).unsqueeze(1)
      enc_output = self.encode(inputs, i_mask)

    return enc_output

  def forward(self, inputs, targets, enc_output=None):
    i_mask = (inputs == self.src_pad_idx).unsqueeze(1)
    if self.has_inputs and enc_output is None:
      i_mask = (inputs == self.src_pad_idx).unsqueeze(1)
      enc_output = self.encode(inputs, i_mask)

    t_mask = (targets == self.trg_pad_idx).unsqueeze(1)
    target_size = targets.size()[1]

    ones = torch.ones(target_size, target_size, dtype=torch.uint8).cuda()
    t_self_mask = torch.triu(ones, diagonal=1).unsqueeze(0) == 1
